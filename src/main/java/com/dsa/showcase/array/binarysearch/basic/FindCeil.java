...
Difficulty: EasyAccuracy: 50.13%Submissions: 33K+Points: 2
...
class FindCeil {
    public static void main(String[] args) {
        ...
        int result = findCeil.findCeil(arr, target);
        System.out.println("Ceil index: " + result);
    }
    public int findCeil(int[] arr, int target) {
        // code here
        int idx = new LowerBound().lowerBound(arr, target);
        if (idx == arr.length) return -1; // ceil doesn't exist
        return idx;
    }
}
