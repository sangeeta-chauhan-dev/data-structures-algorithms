...
class SmallestDivisor {
    ...
    //Time Complexity: O(n log m), where n is the length of the input array and m is the maximum element in the array. The binary search runs in O(log m) and for each divisor, we calculate the sum in O(n).
    ...
    public int getDivisorSum(int[] nums, int divisor) {
        int res = 0;
        for (int num : nums) {
            res += (num + divisor - 1) / divisor; //ceil
        }
        return res;
    }
}